      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          <div className="text-center md:text-left">
            <a href="/" className="inline-flex items-center gap-3 text-2xl font-bold text-blue-600 dark:text-violet-400">
            ...
            <p className="mt-4 text-sm text-gray-600 dark:text-gray-400">
              Empowering professionals to find remote and hybrid career opportunities globally.
            </p>
          </div>
          ...
          <div className="text-center md:text-left">
            ...
            <ul className="space-y-1 text-sm">
              {jobSeekerLinks.map(({ label, href }) => (
                <li key={label}>
                  <a href={href} className="hover:text-blue-600 dark:hover:text-violet-400">{label}</a>
                </li>
              ))}
            </ul>
          </div>
        ...
        <div className="border-t border-gray-300 dark:border-gray-700 pt-6 flex flex-col md:flex-row items-center justify-between text-sm text-gray-600 dark:text-gray-400 space-y-4 md:space-y-0">
          <div className="text-center md:text-left">
            © 2025 CareerLinker. All rights reserved.
          </div>

          <div className="flex gap-4">
            {policyLinks.map(({ label, href }) => (
              <a key={label} href={href} className="hover:text-blue-600 dark:hover:text-violet-400">
                {label}
              </a>
            ))}
          </div>
          ...
          <div className="flex space-x-4">
            {socialLinks.map(({ icon, title, href }) => (
              <a key={title} href={href} title={title} className="hover:text-blue-600 dark:hover:text-violet-400">
                {icon}
              </a>
            ))}
          </div>
